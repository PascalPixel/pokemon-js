...
let Font = () => {
  ...
  for (let glyph in font) {
    ...
    if (glyph === ".notdef") {
      glyphs.push(<missing-glyph key={glyph} d={paths.join("")} />);
    } else {
      glyphs.push(<glyph key={glyph} unicode={glyph} d={paths.join("")} />);
    }
  }

  return (
    <svg>
      <metadata>Copyright (c) 1996 Game Freak and Nintendo</metadata>
      <defs>
        <font id="PokemonGB">
          <font-face
